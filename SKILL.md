...
本 Skill 只出确认单、**HTML 原型**与对接占位。占位 = 字段名 + `USE_MOCK` 开关，**不是**路径 / 错误码 / DDL。数据模型、设计契约、开发计划交回 `requirements-to-dev`，不拆开发模块。

被对方委托时：须已有**已审核需求规格**，且用户已确认下一步交给本 Skill。用规格（及其中的图）走入口④，不要拿未转换的 PRD 当布局依据。**HTML 在对方的选型 / 模型 / 契约 / 计划之前生成。** 规格未审不得假装已委托。

**按对方选型 §2 转工程 = 业务代码**（默认常为 Vue，以对方 §2 为准）：必须交回 `requirements-to-dev`。无已审核开发计划时，用户说「下一步 / 转工程 / 转 Vue / 开始开发」不得在本 Skill 内建工程；先让对方补模型、契约、计划。
...
- 用户叫停打磨：进入本步，不要再改观感
- 转工程：须 `requirements-to-dev` 已审核开发计划之后，按对方 §2（常见 Vue + Element Plus）；分区/图表跟 HTML，**控件用 Element Plus**（见 Vue 模式），再走 Step 5.5 Vue 节；计划未出则交回，禁止在本 Skill 内开工
...
12. 本 Skill 只出确认单、HTML 原型与对接占位（字段名 + mock 开关）；不出路径 / 错误码 / DDL 全文、不拆开发模块。被 `requirements-to-dev` 委托时须规格已审，HTML 在对方选型之前。转工程须对方已有已审核开发计划。后续数据模型 / 设计契约 / 开发计划交回 `requirements-to-dev`。对方未安装则停，不要空口假装已交接。
