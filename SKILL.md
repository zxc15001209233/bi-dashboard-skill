...
- **必须先分区放大再复刻，禁止只凭整图印象转译**：参考图通常只有 1000–2000px 宽，面板级文字（企业名、指标名、状态脚注）在该尺度下不可读，整图直接转译必然漏读或猜错区块数量。做法：把图按区块（顶栏 / 每个面板）切分并放大 4–12 倍逐块读取（可用无头浏览器加载图片后按坐标 `clip` 截图），逐块确认**卡片数量、排布行列、卡内结构、字段构成、状态文案、装饰相对位置**后再动手。
- **布局同构**：数字在左/对比在右、标题在顶/在底、轨线在控件上/下，以分区放大后的图为准。禁止为了「更好读」改成另一种排布；挤了只收字号或给对比列定宽。
...
11. 有参考图时：**布局同构是硬门禁**（卡内结构、标题位置、控件与装饰相对位置、列内疏密）。「不照抄原图颜色」只约束 hex，不授权另排。禁止以可读性为由改排布。生成后必须同区块并排裁切，结构性偏差当场修。仓库已有底图/标题条则用现网壳。
