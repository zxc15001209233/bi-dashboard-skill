...
- 审核结束后**先停**：在对话里用人话列出问题与结论，**等用户 `go` 再改文件**（项目若另有「无 go 不改文件」规则，一并遵守）
- 改完后追加同一份审核文件的「复审」节，或新建 `…-审核-YYYYMMDD.md`，P0 仍在则仍不得声称完成

清单分 HTML / Vue 两节，按当前输出形式执行对应节；「转 Vue」时两节都要过（先对 HTML 原型，再对工程）。

### Step 6：交付与迭代

- **仅当 Step 5.5 结论为通过（无未清 P0）** 才进入本步
...
- [review-checklist.md](review-checklist.md) —— Step 5.5 HTML / Vue 独立审核清单与 P0 定义
