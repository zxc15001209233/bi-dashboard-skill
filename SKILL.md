...
**门禁**
- 有任意 **P0** 未清：禁止说「已完成 / 可以交付」
- 有任意 **P1 / P2** 尚未经用户表态：同样禁止说「已完成 / 可以交付」。P2 不是「不影响读数就可以略过」——必须在对话里用人话列出，等用户明确说接受（不改）或授权改（如 `go`）
- 审核结束后**先停**：列出 P0 / P1 / P2 与结论，**等用户确认后再进入 Step 6 或再改文件**（项目若另有「无 go 不改文件」规则，一并遵守）
- 改完后追加同一份审核文件的「复审」节，或新建 `…-审核-YYYYMMDD.md`；P0 仍在、或 P1/P2 用户尚未表态，仍不得声称完成

清单分 HTML / Vue 两节，按当前输出形式执行对应节；「转 Vue」时两节都要过（先对 HTML 原型，再对工程）。

### Step 6：交付与迭代

- **仅当** Step 5.5 无未清 P0，**且** P1 / P2 已由用户确认（接受不改，或已按授权改完并复审）才进入本步
...
- [review-checklist.md](review-checklist.md) —— Step 5.5 HTML / Vue 独立审核清单；P0 挡交付，P1/P2 须用户确认
