...
| 参考图/截图 | **复刻模式**（不是套模板皮）：先分区放大再复刻；**顶栏死门 + 卡内死门**写不出就停问；读图冲突停问；图宽 < 1600 须声明精度不足；确认单未发不得因 `go` 抢跑；禁止从 `screen.html` 的 panel/KPI 默认脸起稿；主数字色相跟图（hex 仍用 token）；生成后必须写**区块视觉差清单**，只核块在/数对或只交 JS 校验不算过 |
