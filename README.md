...
| 参考图/截图 | **必须先分区放大再复刻**（按顶栏/每个面板切分并放大 4–12 倍，逐块确认卡片数量、排布行列、**卡内结构**、字段与状态文案、**字号层级**），禁止只凭整图印象转译；**布局同构是硬门禁**（不照抄 hex ≠ 可以另排或缩小标题/主数字一档）；生成后须与参考图同区块并排裁切，结构性偏差与字号差一档当场修；复刻后用比例文字反述确认（如"左列约占 24%"） |
